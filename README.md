...
Таким образом, мы можем писать задание для дрона в текстовом файле commands.txt, который мы поместим в ту же папку.
...
Так вы можете реализовать собственный аналог Scratch, но придется прописывать больше логики при чтения файла (пример - как реализован сон в файле выше).

Для запуска данного кода нужно запустить командную строку, перейти в папку с файлами [commands.py](examples/commands.py) и [commands.txt](examples/commands.txt) с помощью cd, а затем запустить файл commands.py с помощью py, примерно как в этом примере:
...
Принимает один необязательный аргумент - максимальное время ожидания в секундах.
```python
from djitellopy import TelloSwarm # импорт класса управления роем
...
____
# DJI Tello
Обе модели дронов имеют схожие характеристики - систему позиционирования, камеру на 5 мегапикселей, 13 минут полета от одного заряда, индикатор состояния, защиту вокруг лопастей.
____
## Комплектация
* Дрон
* Аккумулятор
...
* Запасные лопасти
* Комплект Mission Pads (только Tello EDU)

В комплекте не поставляется блок зарядного устройства. Аккумулятор принимает ток до 1А при напряжении 5В. Рекомендуется использовать блок на 5В с силой тока не ниже 1,5А (больше - можно, контроллер ограничит ток допустимым уровнем). Зарядка от USB порта ПК имеет слишком малый ток и не рекомендуется.
____
## Техника безопасности и правила эксплуатации
* Запрещается летать слишком близко к людям и животным или над ними
* Запрещается трогать дрон до полной остановки моторов
* Запрещается использовать дрон на улице в ветряную или влажную погоду
* Запрещается заряжать дрон сразу после длительного использования - это может привести к критическому перегреву аккумулятора
* Разрешается использовать дрон только на относительно открытых площадках, где вы сможете избегать столкновений с препятствиями, людьми или животными
* Запрещается использовать дрон на местности с резким перепадом высоты (например, вылетать им в окно здания) - это может привести к сбою системы позиционирования
* Запрещается летать рядом с линиями электропередач и источниками огня
* Перед запуском необходимо убедиться, что лопасти, защитные элементы и аккумулятор надежно закреплены
* Допустимый температурный режим использования дрона - от 0 до 40 градусов Цельсия
* Во время полета недопустимо терять дрон из поля зрения
____
## Хранение
Необходимо хранить дрон и аккумулятор в рамках допустимого температурного режима их использования - от 0 до 40 градусов.

Для более длительной службы аккумулятора извлекайте его после окончания использования дрона.

Если дрон не будет использоваться длительное время - аккумулятор необходимо зарядить приблизительно до 65%.

Не оставляйте на длительный период полностью разряженный аккумулятор, это быстро приведет к критичной потере его емкости.
____
## Mission Pad
...
Чем больше высота, тем шире область видимости. Однако, на высоте более 120 см начинаются ошибки определения координат, а на высоте более 150 см определение становится непостоянным и ухудшается с увеличением высоты вплоть до полной невозможности определить коврик на 180 см. На слишком низкой высоте коврик может физически не поместиться в область видимости камеры. Оптимальная высота для определения - 35-115 см.
...
1) Необходимо скачать последний Python 3.x для своей операционной системы с официального сайта:
...
Проблемы могут возникнуть только если на используемом ПК уже имеется иная версия Python - pip может установить библиотеку в её дирректорию.

Все необходимое теперь имеется на ПК, остается выбрать только среду для разработки (редактор, в котором вы будете писать код).

Это может быть буквально любой редактор, зависит от целей и предпочтений. Все они имеют плюсы и минусы.

Полноценная IDE, такая как PyCharm, дает максимальную помощь в написании кода - подсветка ошибок синтаксиса, подсказки, автодополнение. Она максимально упростит и ускорит написание кода, но это не всегда хорошо - во время учёбы она может отнять некоторую часть самостоятельности - когда IDE подсвечивает ошибки синтаксиса за этим не нужно следить самому. Когда IDE помогает автодополнением - не нужно помнить названия методов. Так же PyCharm требует достаточно мощный ПК. Новые версии требуют в обязательном порядке 64 битную ОС.

Если выбор в пользу PyCharm - абсолютно бесплатная версия Community доступна на сайте её разработчиков JetBrains.

![pycharmdown](images/pycharmdown.png)

При создании нового проекта (File - New Project) нужно ввести имя проекта, выбрать установленную выше версию Python и отметить пункт Inherit global site-packages (иначе не будет видно установленную библиотеку djitellopy и автоматически будет установлена более старая версия из репозитория PyPI, а она не всегда работает).

![pycharmproj](images/pycharmproj.png)

Теперь можно создавать новый .py файл (File - New - Python File) и писать код. Запуск программы произволится нажатием ctrl+shift+F10 или кнопкой в правом верхнем углу экрана.

Полная противоположность IDE - разработка программы в обычном блокноте. Он не даст никакой помощи и подсказок, зато работает на любом, даже очень слабом ПК. И главное - не разовьет никакой зависимости от "помощи IDE", что полезно при сдаче ЕГЭ по информатике - там код придётся писать ручкой на листочке и помощи ждать будет неоткуда.

Просто создайте текстовый .txt файл, вместо стандартного расширения укажите .py, откройте его блокнотом и пишите код. При сохранении убедитесь что выбрана кодировка UTF-8. Запуск такой программы производится из командной строки.

Сначала с помощью команды cd нужно перейти в папку, где сохранен файл с кодом, а затем нужно запустить его с помощью команды py:

![cmdpy](images/cmdpy.png)

Но можно выбрать и иные варианты, например, Visual Studio Code с аддоном Python или уже немного устаревший Notepad++. На этом список не ограничивается.

Все перечисленное выше можно абсолютно бесплатно скачать с сайтов разработчиков данного ПО.
____
## Объединение в рой
Единственный простой, но не единственный существующий, способ объединить несколько дронов в рой - подключить несколько Tello EDU к одной локальной сети по Wi-Fi с помощью команды [connect_to_wifi(ssid, password)](https://github.com/Hellsingoff/DJITelloPy#connect_to_wifissid-password) и узнать какие IP они получили. Рекомендуется закрепить статические IP адреса за мак адресами дронов в настройках Wi-Fi роутера чтобы они были постоянными.

Для Ryze этот метод не подходит.